...
- **The ChatGPT subscriptions card** (#2453) on
  ...
  and remove it. A reload shows the same pending code, and the page updates
  by itself when the sign-in completes. A **ChatGPT subscription** row names
...
- **`/start` and the agent form say when a named ChatGPT subscription
  ...
  subscription is disconnected, revoked, expired or spent, or on a
  deployment with no credential broker, `/start` no longer implies the
