...
| `MACHINE_OWNER_ENABLED` | `false` | — | **Leave this off.** It is the gate on a feature Fountain is still building: one owner process per sandbox, which will become the only writer of that sandbox's state. In this release the owner reads, and it writes nothing at all. A `true` starts one process per active sandbox, and each one answers the same question — who is on this machine — that Fountain answers without it. So the flag costs a process per machine and changes no behaviour. Turn it on only on a test instance, and only to watch it. |
