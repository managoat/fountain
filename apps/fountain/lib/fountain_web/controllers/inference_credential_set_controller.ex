  ...
  operation(:delete,
    ...
    description:
      "Refused for the default set: something has to answer which credential " <>
        "runs this account. Promote another first. Deleting a set removes its " <>
        "credentials; conversations bound to that source refuse to resume " <>
        "with inference_source_changed.",
    parameters: [id: [in: :path, type: :string, required: true]],
    responses: [
      no_content: "Deleted",
      forbidden: {"Insufficient scope", "application/json", Schemas.Error},
    ...
    with %{} = set <- InferenceCredentials.get_set(id, user.id) || {:error, :not_found},
      ...
      {:error, :is_default} ->
        ...
        |> json(%{
          ...
          message: "the default credential set cannot be deleted; promote another one first",
          reason: "is_default"
        })
