defmodule FountainSlack.DocsTest do
  @moduledoc """
  ...
  describe "this app's slice" do
    test "every page the nav names exists on disk" do
      FountainSlack.Docs |> Checks.nav_pages_exist() |> assert_sound()
    end

    test "every page on disk is named in the nav" do
      FountainSlack.Docs |> Checks.pages_on_disk_named() |> assert_sound()
    end

    test "every page resolves with a title and a body" do
      for slug <- FountainSlack.Docs.slugs() do
        assert {:ok, %{title: title, body: body}} = FountainSlack.Docs.get(slug)
    ...
    test "no authoring syntax survived preprocessing" do
      FountainSlack.Docs |> Checks.no_leftover_syntax() |> assert_sound()
    end

    test "the search index has one entry per page, and its headings resolve" do
      FountainSlack.Docs |> Checks.search_index_complete() |> assert_sound()
      FountainSlack.Docs |> Checks.search_index_headings_resolve() |> assert_sound()
      FountainSlack.Docs |> Checks.search_index_json_safe() |> assert_sound()
    end
