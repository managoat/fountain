defmodule FountainWeb.CredentialSetPolicyRoundtripTest do
  ...
  test "API, versions and apply round-trip the existing null/empty/finite contract", %{conn: conn} do
    ...
    {:ok, set} = InferenceCredentials.create_set(user.id, "Round trip")
    ...
    for {ids, mode} <- modes do
      payload = %{
        "name" => "credential-policy-#{System.unique_integer([:positive])}",
      ...
      id = created["data"]["id"]
      assert created["data"]["allowed_inference_credential_ids"] == ids
      refute Map.has_key?(created["data"], "inference_credential_access")
      assert Agents.get_agent(id, user.id).inference_credential_access == mode

      read = conn |> get(~p"/api/agents/#{id}") |> json_response(200)
      assert read["data"]["allowed_inference_credential_ids"] == ids
      updated = conn |> put_json(~p"/api/agents/#{id}", read["data"]) |> json_response(200)
      assert updated["data"]["allowed_inference_credential_ids"] == ids

      version = conn |> get(~p"/api/agents/#{id}/versions/1") |> json_response(200)
      assert Map.fetch!(version["data"]["config"], "allowed_inference_credential_ids") == ids
      refute Map.has_key?(version["data"], "inference_credential_access")

      # Force a policy change first, so apply must restore the serialized shape.
      {:ok, _} =
        Agents.update_agent(Agents.get_agent(id, user.id), %{
  ...
  test "omitted API/apply policy preserves deny-all; explicit null opens only the tenant policy",
    ...
    {_key, raw_key} = insert_api_key(user)
    conn = authed_with_key(conn, raw_key)
    agent = insert_agent(user_id: user.id, allowed_inference_credential_ids: [])

    conn
    |> put_json(~p"/api/agents/#{agent.id}", %{"description" => "API edit"})
    |> json_response(200)

    assert Agents.get_agent(agent.id, user.id).allowed_inference_credential_ids == []
    ...
    conn
    |> put_json(~p"/api/agents/#{agent.id}", %{"allowed_inference_credential_ids" => nil})
    |> json_response(200)

    assert Agents.get_agent(agent.id, user.id).inference_credential_access ==
             "all_tenant_credential_sets"
  end
end
