...
`Fountain.MachineRegistry`; five columns on `sandboxes`; one retryable
refusal added to every transient-error vocabulary at once (#2307 constraint 6
lists them); one GenServer per *active* machine, idle-stopping, not one per
