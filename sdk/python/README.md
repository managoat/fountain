...
```python
client.run("Run the approved plan.", agent="reposage",
           client_request_id="plan-7-step-3").result()
client.resume(conversation_id).send("And the next step.",
                                   client_request_id="plan-7-step-4").result()
```
