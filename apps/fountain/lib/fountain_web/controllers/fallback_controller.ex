defmodule FountainWeb.FallbackController do
  ...
  def call(conn, {:error, :inference_source_changed}) do
    conn
    |> put_status(:conflict)
    |> json(%{
  ...
  # The ChatGPT subscription a credential set names cannot serve this codex
  # run (ADR 0060 decision 4). 409, like `inference_source_changed`: the
  # request is fine and the state of something the caller owns is not.
  # Nothing was substituted, and the message says so. Ids, the name the
  # tenant chose, a reason atom and a time; never a token.
  def call(conn, {:error, {:chatgpt_grant_unusable, %{reason: reason} = detail}}) do
    conn
    |> put_status(:conflict)
    |> json(%{
  ...
  def call(conn, {:error, :inference_credential_unusable}) do
    ...
    |> json(%{
      ...
      message:
        "the selected credential set has no credential this runtime can use; add one or select another set"
    })
  end

  def call(conn, {:error, :inference_credential_conflict}) do
    conn
    |> put_status(:unprocessable_entity)
    |> json(%{
      error: "inference_credential_conflict",
      message:
        "multiple authentication inputs compete for this runtime; keep one credential source before launching"
    })
  end

  def call(conn, {:error, :codex_inference_conflict}) do
    conn
    |> put_status(:conflict)
    |> json(%{
