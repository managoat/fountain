...
| `MACHINE_OWNER_ENABLED` | `false` | — | **Reserved. It has no effect in this release. Leave it unset.** It is the gate on a feature Fountain is still building (ADR 0058): one owner process per sandbox, which is to become the only writer of that sandbox's state. The owner process exists in this release, and nothing starts one. So a `true` changes nothing at all, and it starts no process and writes nothing. A later release gives this variable an effect, and documents it here. |
