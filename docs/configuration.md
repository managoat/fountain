...
| `MACHINE_OWNER_ENABLED` | `false` | — | **Leave it unset unless you have a reason not to.** It is the gate on a feature Fountain is still building (ADR 0058): one owner process per sandbox, which is to become the only writer of that sandbox's state. In this release it chooses *where* a sandbox destroy runs, not what it does. With it unset, a destroy runs on the process that asked for it. With it `true`, a destroy runs inside that sandbox's owner process, so two destroys of one machine queue behind one another instead of racing for its lease, and a caller waits at most 20 seconds for that queue before it is told the machine is unavailable and can retry. Same steps, same database writes, same audit events either way — the safety comes from the lease on the row, which both paths take. On either side, a destroy that finds another one already holding the machine waits about 5 seconds and then answers `sandbox_unavailable` rather than blocking the request. Every other machine operation is unaffected in this release. |
