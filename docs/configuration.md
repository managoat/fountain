...
| `MACHINE_OWNER_ENABLED` | `false` | — | **Leave it unset unless you have a reason not to.** It is the gate on a feature Fountain is still building (ADR 0058): one owner process per sandbox, which is to become the only writer of that sandbox's state. In this release it chooses *where* a sandbox destroy, park, wake, turn admission, attach or terminate runs, not what it does. With it unset, each runs on the process that asked for it. With it `true`, each runs inside that sandbox's owner process, so two operations on one machine queue behind one another instead of racing for its lease; a caller waits at most 20 seconds for a destroy, a wake, a turn admission, an attach or a terminate and 60 for a park before it is told the machine is unavailable and can retry, and an attach, a terminate, a wake or a turn admission the owner reaches only after its caller gave up is refused rather than run for nobody. Same steps, same database writes, same audit events either way — the safety comes from the lease on the row, which both paths take, so two prompts wake a parked machine once whichever side of the gate you are on. On either side, an operation that finds another one already holding the machine waits about 5 seconds and then answers `sandbox_unavailable` rather than blocking the request, and an operation still making progress keeps its hold on the machine however long it takes. **Building** a sandbox always runs on the process that asked for it, whatever this is set to — the work is that conversation's own setup pipeline and it can take minutes — and it takes the same lease as the rest, which is what keeps two processes from building one sandbox twice. **Admitting a turn** takes no lease of its own — it is one database transaction under the sandbox's lock, and a lease would cost two row writes per prompt for nothing — but it refuses a machine whose owner holds a live lease, waiting about 5 seconds first, on either side of this setting; and a turn *ending* always runs on the process that ended it. Every other machine operation is unaffected in this release. |
