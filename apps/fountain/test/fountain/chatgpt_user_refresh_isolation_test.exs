defmodule Fountain.ChatGPTUserRefreshIsolationTest do
  ...
  # Two grants of ONE user are the case ADR 0060 adds: the refresh lock, the
  # coordinator key and the fence are all per grant, so they renew as
  # independently as two users' grants do.
  for {owners, second_owner} <- [{"two users", 1}, {"two grants of one user", 0}] do
    test "#{owners} and the platform renew independently while a same-grant contender waits",
         ctx do
      renew_independently(ctx, Enum.at(ctx.users, unquote(second_owner)))
    end
  end

  defp renew_independently(ctx, second_user) do
    first_user = hd(ctx.users)
  ...
  for mutation <- [:disconnect, :reconnect, :delete, :raw_reconnect, :suspend],
    ...
    test "#{mutation} commits while an independent user #{response} response is pending", ctx do
      ...
      try do
        ...
        refute Repo.exists?(
                 from(e in Event,
                   where: e.action == "chatgpt_grant.reconnect_required" and e.user_id == ^user.id
                 )
               )
      after
        Task.shutdown(holder, :brutal_kill)
