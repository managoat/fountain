...
- **Claude Opus 5.5 (`anthropic/claude-opus-5-5`) is a suggested model**
  (#PRNUM). Platform-key turns on it are priced at Anthropic's list rates. Opus 5
...
- **Claude conversations on a Claude.ai subscription token wake and
  provision about 30 seconds faster** (#PRNUM). Since 2026-09-12, sandbox setup ran a
...
- **Claude Fable 5.1 works on a Claude.ai subscription token, and on a
  sandbox's first conversation** (#PRNUM). Before, those turns failed with
  "Invalid value for config option model".
- **Platform-key turns on Claude Fable 5.1 are priced at Fable's rate**
  (#PRNUM). They were priced at the Opus rate, half their cost.

### Changed

- **The Claude adapter moves to claude-agent-acp 0.81.2** (#PRNUM). Its `opus`
