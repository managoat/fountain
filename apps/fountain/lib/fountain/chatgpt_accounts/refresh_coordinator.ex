defmodule Fountain.ChatGPTAccounts.RefreshCoordinator do
  ...
  # Defining `handle_info/2` removes the clause `use GenServer` generates, so
  # without this any other message would take the coordinator, and every
  # worker under it, down (#2380). The shape only, never the payload: this
  # process sits beside token renewal.
  def handle_info(message, state) do
    Logger.warning("chatgpt refresh coordinator: unexpected message #{shape(message)}; ignoring")
    {:noreply, state}
  end

  defp shape(message) when is_tuple(message) and is_atom(elem(message, 0)),
    do: "#{inspect(elem(message, 0))}/#{tuple_size(message)}"

  defp shape(message) when is_tuple(message), do: "a tuple/#{tuple_size(message)}"
  defp shape(message) when is_atom(message), do: inspect(message)
  defp shape(_message), do: "a non-tuple term"
