...
- A prompt that arrives while a computer is being reset or deleted no longer
  ...
  stands down, tears down what it made and leaves the reset to finish.
...
- A conversation whose computer the provider says is gone now retires that
  ...
  one entry in the activity trail, rather than a status written in passing.
...
- The absolute thirty-minute ceiling on building a computer is unchanged, and
  ...
  A build Fountain cannot retire at the ceiling is now left for the hourly
  cleanup pass instead of having its process killed, which is what used to make
  a second computer get built.
